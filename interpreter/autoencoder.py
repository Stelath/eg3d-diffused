...
class AutoencoderKLConfig():
    def __init__(
        ...
        self.out_ch = out_ch
        self.ch = ch
        self.ch_mult = ch_mult # num_down = len(ch_mult)-1
    ...
    def to_dict(self):
        d = {}
        d['double_z'] = self.double_z
        d['z_channels'] = self.z_channels
        d['resolution'] = self.resolution
        d['in_channels'] = self.in_channels
        d['out_ch'] = self.out_ch
        d['ch'] = self.ch
        d['ch_mult'] = self.ch_mult # num_down = len(ch_mult)-1
        d['num_res_blocks'] = self.num_res_blocks
        d['attn_resolutions'] = self.attn_resolutions
...
class AutoencoderKL(pl.LightningModule):
    def __init__(self,
        ...
        self.encoder = Encoder(**(ddconfig.to_dict()))
        self.decoder = Decoder(**(ddconfig.to_dict()))
        self.loss = LPIPSWithDiscriminator(**ddconfig.loss_config)
