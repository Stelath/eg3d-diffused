...
def encoder_evaluate(config, epoch, model, eval_dataset):
    pass

def evaluate(config, epoch, pipeline, eg3d, eval_dataset, save=True):
    ...
    predicted_lvs = pipeline(
        imgs,
    ).images

    imgs = imgs.numpy().transpose(0, 2, 3, 1)
    predicted_imgs = eg3d.generate_imgs(predicted_lvs).cpu().numpy()
